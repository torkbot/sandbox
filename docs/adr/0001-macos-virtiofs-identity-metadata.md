...
For every write and truncation, virtio-fs first replaces the metadata record with file capabilities removed, then mutates the contents. The FUSE killpriv request flag controls whether the same replacement also removes the required setuid/setgid bits; it does not control capability removal. Failure to remove privilege metadata prevents the content mutation. If the later content operation fails, the conservative privilege removal remains.
...
macOS cannot atomically create a directory entry and attach its metadata record. Sandbox creates the requested final path and attaches the complete record before reporting guest success. Regular-file creation first attempts an exclusive host create so the backend knows whether the request owns the inode. If a host actor populates a path after the guest's negative lookup, guest `O_CREAT` without `O_EXCL` opens that existing inode without adopting it; guest `O_EXCL` still reports `EEXIST`. If metadata attachment fails, creation fails but Sandbox leaves the final pathname untouched. macOS has no pathname unlink that is atomically conditional on inode identity, so cleanup could delete a host replacement installed during the failure window. A sandbox-host crash or metadata failure can therefore leave a native entry; this is accepted rather than introducing temporary-name recovery, scanning, journaling, or sidecar state.
...
- Code comments must be placed at the non-obvious invariant boundaries: the record as adoption marker, attribute-not-found as the only per-inode native signal, owner-only backing modes, privilege removal before content mutation, refusing pathname cleanup after metadata attachment failure, and combined-record updates relying on the single synchronous virtio-fs worker. Comments should explain why the ordering or split exists rather than restating the operations.
- End-to-end tests must express the public guest/host behavior, while focused backend tests cover parsing, error propagation, concurrency, creation failure, and xattr namespace rules.
