...
test("outbound policy creates the guest network device and default route", async (t) => {
  if (!requireVmLaunchSupport(t)) {
    return;
  }

  const vm = await spawnSandbox({
    name: "outbound-network-device",
...
test("outbound default deny returns a deterministic HTTP denial", async (t) => {
  if (!requireVmLaunchSupport(t)) {
    return;
  }
